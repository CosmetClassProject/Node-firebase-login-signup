...
(function onload(){
    const config = {
      ...
      const txtEmail = document.getElementById("raw_name");
      const txtPassword = document.getElementById("raw_pass");
    ...
    btnLogin.addEventListener('click', e => {
          console.log("in");
            var email = txtEmail.value;//get the email
            var pass = txtPassword.value;//get the password
            const auth = firebase.auth();
            //return a promise to sign in the user
            const promise = auth.signInWithEmailAndPassword(email, pass);//sign the user in using the email and password
